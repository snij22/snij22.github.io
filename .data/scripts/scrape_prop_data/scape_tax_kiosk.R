...
parcel_nums <- st_read(dsn = parcel_polys_fp) %>%
  st_drop_geometry() %>%
  select(PARCEL_) %>%
...
franklin_hyper <- remDr$findElement(
  using = "id",
...
parcel_nums <- parcel_nums %>%
  mutate(
    map_num = str_split(PARCEL_, "-", simplify = T)[, 1],
...
existing <- read_csv(here::here(".data/assess_ts_1.csv")) %>%
  ...
  filter(`Map-Lot-Sub` != "") %>%
  mutate(
    map_num = str_split_i(`Map-Lot-Sub`, "-", 1) %>% str_sub(., -3),
...
for (i in 1:nrow(re_try)) {
  ...
  search_button_1 <- remDr$findElement(
    using = "id",
  ...
  Sys.sleep(3)
  # wrap in try catch
  parcel_hyper <- tryCatch(
    remDr$findElement(
      using = "id",
      value = "ctl00_ContentPlaceHolder1_TaxBillsByPID_ctl02_hyperDetails"
    ),
    error = function(e) {
      skip_to_next <<- TRUE
    }
  ...
  full_tab <- read_html(page_results) %>%
    html_table() %>%
    keep(~ ncol(.x) == 4) %>%
    map(read_a_tab_cell) %>%
    bind_rows()
